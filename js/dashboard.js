...
let uploadedFiles = []; // new files to upload (File objects)
let existingImages = []; // already-uploaded URLs (strings)
...
function initNavigation() {
  ...
  document
    .querySelector('[data-page="pedidos"]:not(.nav-item)')
  ...
  document.querySelectorAll("#ordersFilters .filter-chip").forEach((chip) => {
    chip.addEventListener("click", () => {
      document
        .querySelectorAll("#ordersFilters .filter-chip")
...
// Apply all active filters to allOrders
function getFilteredOrders() {
  ...
  // Client filter
  const clientQ = document
    .getElementById("filterCliente")
    ?.value.trim()
    .toLowerCase();
  if (clientQ)
    list = list.filter((o) =>
      (o.clienteNome || "").toLowerCase().includes(clientQ),
    );
...
function renderOrdersTable() {
  ...
  tbody.innerHTML = list
    .map((o) => {
      // Support both old format (produto) and new format (itens[])
      const numItens = o.itens?.length ?? 1;
      const total = o.total ?? o.produto?.preco ?? 0;
      const codigo = o.codigo || "#" + o.id.slice(-6).toUpperCase();

      return `
    <tr style="cursor:pointer;" data-order-id="${o.id}" title="Clique para ver detalhes">
      <td data-label="Código"><span class="order-code">${esc(codigo)}</span></td>
      ...
      <td data-label="Itens" class="td-hide-mobile" style="color:var(--text-secondary);">${numItens} item${numItens !== 1 ? "s" : ""}</td>
      <td data-label="Total"  class="td-hide-mobile" style="font-weight:700;color:var(--navy);">R$ ${fmtPrice(total)}</td>
...
function renderRecentOrders() {
  ...
  if (!list.length) {
    tbody.innerHTML = `<tr><td colspan="5"><div class="empty-state"><p>Nenhum pedido ainda.</p></div></td></tr>`;
    return;
  }
  tbody.innerHTML = list
    .map((o) => {
      ...
      return `
    <tr>
      <td><span class="order-code">${esc(codigo)}</span></td>
...
function openOrderDetail(order) {
  ...
  const codigo = order.codigo || "#" + order.id.slice(-6).toUpperCase();
  setText("odCodigo", `Pedido ${codigo}`);
  setText("odNome", order.clienteNome || "—");
  setText("odObs", order.observacoes || "—");
  setText(
    "odData",
    order.criadoEm?.toDate ? fmtDate(order.criadoEm.toDate()) : "—",
  );
  ...
  // Build items list
  const itensList = document.getElementById("odItensList");
  if (itensList) {
    // New format: itens[]
    if (order.itens?.length) {
      itensList.innerHTML = order.itens
        .map(
          (item, idx) => `
        <div class="od-item">
          <div class="od-item-info">
            <div class="od-item-name">${esc(item.nome || "—")}</div>
            <div class="od-item-size">${item.tamanho && item.tamanho !== "—" ? `Tam. ${item.tamanho} · ` : ""}Qtd ${item.quantidade || 1}</div>
          </div>
          <span class="od-item-qty">x${item.quantidade || 1}</span>
          <div style="display:flex;flex-direction:column;align-items:flex-end;gap:3px;">
            <div style="font-size:9px;font-weight:700;text-transform:uppercase;letter-spacing:0.06em;color:var(--text-muted);">Preço unit.</div>
            <div style="display:flex;align-items:center;gap:4px;">
              <span style="font-size:12px;color:var(--text-muted);">R$</span>
              <input type="number" class="od-price-input" data-idx="${idx}"
                     value="${Number(item.precoFinal ?? item.preco).toFixed(2)}"
                     step="0.01" min="0" style="width:90px;">
            </div>
          </div>
          <div class="od-item-subtotal" id="subtotal-${idx}">
            R$ ${fmtPrice((item.precoFinal ?? item.preco) * (item.quantidade || 1))}
          </div>
        </div>`,
        )
        .join("");

      // Live update total on price change
      itensList.querySelectorAll(".od-price-input").forEach((input) => {
        input.addEventListener("input", () => recalcTotal(order));
      });
    } else {
      // Legacy format: produto single
      const preco = order.produto?.preco || 0;
      itensList.innerHTML = `
        <div class="od-item">
          <div class="od-item-info">
            <div class="od-item-name">${esc(order.produto?.nome || "—")}</div>
            <div class="od-item-size">${order.tamanho && order.tamanho !== "—" ? `Tam. ${order.tamanho} · ` : ""}Qtd ${order.quantidade || 1}</div>
          </div>
          <span class="od-item-qty">x${order.quantidade || 1}</span>
          <div style="display:flex;flex-direction:column;align-items:flex-end;gap:3px;">
            <div style="font-size:9px;font-weight:700;text-transform:uppercase;letter-spacing:0.06em;color:var(--text-muted);">Preço unit.</div>
            <div style="display:flex;align-items:center;gap:4px;">
              <span style="font-size:12px;color:var(--text-muted);">R$</span>
              <input type="number" class="od-price-input" data-idx="0"
                     value="${Number(preco).toFixed(2)}" step="0.01" min="0" style="width:90px;">
            </div>
          </div>
          <div class="od-item-subtotal" id="subtotal-0">R$ ${fmtPrice(preco * (order.quantidade || 1))}</div>
        </div>`;
      itensList.querySelectorAll(".od-price-input").forEach((input) => {
        input.addEventListener("input", () => recalcTotal(order));
      });
...
function recalcTotal(order) {
  const inputs = document.querySelectorAll(".od-price-input");
  let total = 0;
  inputs.forEach((input, idx) => {
    const preco = parseFloat(input.value) || 0;
    const qtd = order.itens?.[idx]?.quantidade || order.quantidade || 1;
    const sub = preco * qtd;
    total += sub;
    const subEl = document.getElementById(`subtotal-${idx}`);
    if (subEl) subEl.textContent = `R$ ${fmtPrice(sub)}`;
  });
  setText("odTotal", `R$ ${fmtPrice(total)}`);
}
...
function renderVendedorasReport() {
  ...
  grid.innerHTML = VENDEDORAS.map((nome) => {
    ...
    const total = pedidos.reduce(
      (s, o) => s + (Number(o.produto?.preco) || 0),
      0,
    );
  ...
  grid.querySelectorAll("[data-vend]").forEach((card) => {
    card.addEventListener("click", () =>
      openVendedoraDetail(card.dataset.vend),
    );
  });
  ...
  if (!com.length) {
    tbody.innerHTML = `<tr><td colspan="5"><div class="empty-state"><p>Nenhum pedido atribuído ainda.</p></div></td></tr>`;
    return;
  }
  tbody.innerHTML = com
    .map(
...
function openVendedoraDetail(nome) {
  ...
  setText(
    "vdSub",
    `${pedidos.length} pedido${pedidos.length !== 1 ? "s" : ""} · R$ ${fmtPrice(total)} em vendas`,
  );
  ...
  if (Object.keys(statusCounts).length > 0) {
    chartInstances.vdStatus = makeDoughnut(
      "chartVdStatus",
    ...
    if (ctx) {
      ...
      p.innerHTML = `<div style="display:flex;align-items:center;justify-content:center;height:160px;font-size:13px;color:var(--text-muted);">Sem dados</div>`;
    }
  }

  // Monthly bar (last 6 months)
  const mData = getMonthlyData(pedidos);
  chartInstances.vdMonthly = makeBar(
    "chartVdMonthly",
    Object.keys(mData),
    Object.values(mData).map((d) => d.valor),
    c,
  );
...
function renderRelatorio() {
  ...
  const thisWeek = weekOrders.reduce(
    (s, o) => s + (Number(o.produto?.preco) || 0),
    0,
  );
  const thisMonth = monthOrders.reduce(
    (s, o) => s + (Number(o.produto?.preco) || 0),
    0,
  );
  ...
  setText(
    "reportWeekSub",
    `${weekOrders.length} pedido${weekOrders.length !== 1 ? "s" : ""}`,
  );
  ...
  // Destroy old charts
  ["monthly", "statusChart", "vendors", "weekly"].forEach((k) => {
    chartInstances[k]?.destroy();
    delete chartInstances[k];
  });
  ...
  chartInstances.monthly = makeBar(
    "chartMonthly",
    Object.keys(mData),
    Object.values(mData).map((d) => d.valor),
  );
  ...
  chartInstances.statusChart = makeDoughnut(
    "chartStatus",
    Object.keys(sCounts),
    Object.values(sCounts),
  );
  ...
  allOrders
    .filter((o) => o.vendedora)
    .forEach((o) => {
      if (vData[o.vendedora] !== undefined)
        vData[o.vendedora] += Number(o.produto?.preco) || 0;
    });
  chartInstances.vendors = makeBarMultiColor(
    "chartVendedoras",
    Object.keys(vData),
    Object.values(vData),
    Object.keys(vData).map((v) => VEND_COLORS[v] || "#1D3C8A"),
  );

  // Weekly line
  const wData = getWeeklyData();
  chartInstances.weekly = makeLine(
    "chartWeekly",
    Object.keys(wData),
    Object.values(wData).map((d) => d.count),
  );
}
...
function getMonthlyData(orders = allOrders) {
  ...
  for (let i = 5; i >= 0; i--) {
    ...
    data[`${names[d.getMonth()]}/${String(d.getFullYear()).slice(2)}`] = {
      valor: 0,
      count: 0,
    };
  }
  orders.forEach((o) => {
    ...
    if (mAgo < 6) {
      ...
      if (data[key]) {
        data[key].valor += Number(o.produto?.preco) || 0;
        data[key].count++;
      }
    }
  });
...
function getWeeklyData() {
  ...
  allOrders.forEach((o) => {
    ...
    if (keys[weekIdx]) {
      data[keys[weekIdx]].valor += Number(o.produto?.preco) || 0;
      data[keys[weekIdx]].count++;
    }
  });
  return data;
}
...
const CHART_DEFAULTS = {
  ...
  scales: {
    y: {
      ...
      ticks: { font: { size: 10 }, color: "#8A9BB8" },
    },
    x: {
      grid: { display: false },
      ticks: { font: { size: 10 }, color: "#8A9BB8" },
      ...
      datasets: [
        {
          data,
          borderColor: "#1D3C8A",
          backgroundColor: "rgba(29,60,138,0.08)",
          borderWidth: 2,
          pointRadius: 4,
          pointBackgroundColor: "#1D3C8A",
          fill: true,
          tension: 0.3,
        },
      ],
  ...
  grid.innerHTML = allProducts
    ...
    <div class="admin-card">
      <div class="admin-card-img">
        ${
          p.imagemUrl
            ? `<img src="${p.imagemUrl}" alt="${esc(p.nome)}" loading="lazy">`
...
function initModals() {
  ...
  // Order detail
  document
    .getElementById("closeOrderDetail")
    ?.addEventListener("click", () => closeModal("orderDetailModal"));
  document
    .getElementById("cancelOrderDetail")
    ?.addEventListener("click", () => closeModal("orderDetailModal"));
  document
    .getElementById("saveOrderDetail")
    ?.addEventListener("click", saveOrderDetail);
  document
    .getElementById("orderDetailModal")
    ?.addEventListener("click", (e) => {
      if (e.target.id === "orderDetailModal") closeModal("orderDetailModal");
    });

  // Vendedora detail
  document
    .getElementById("closeVendedoraDetail")
    ?.addEventListener("click", () => closeModal("vendedoraDetailModal"));
  document
    .getElementById("cancelVendedoraDetail")
    ?.addEventListener("click", () => closeModal("vendedoraDetailModal"));
  document
    .getElementById("vendedoraDetailModal")
    ?.addEventListener("click", (e) => {
      if (e.target.id === "vendedoraDetailModal")
        closeModal("vendedoraDetailModal");
    });
  ...
  const uz = document.getElementById("uploadZone");
    ...
    files.forEach((f) => {
      if (f.size <= 5 * 1024 * 1024) addUploadFile(f);
      else showToast(`${f.name} muito grande (máx 5MB)`, "error");
    });
    fi.value = "";
  });
  ...
  uz?.addEventListener("drop", (e) => {
    ...
    Array.from(e.dataTransfer.files).forEach((f) => {
      if (f.type.startsWith("image/")) addUploadFile(f);
    });
  });
}
...
async function saveOrderDetail() {
  ...
  try {
    ...
    // Save updated prices
    if (order?.itens?.length) {
      const updatedItens = order.itens.map((item, idx) => ({
        ...item,
        precoFinal:
          parseFloat(inputs[idx]?.value) || item.precoFinal || item.preco,
      }));
      updates.itens = updatedItens;
      updates.total = updatedItens.reduce(
        (s, i) => s + (i.precoFinal || i.preco) * (i.quantidade || 1),
        0,
      );
    } else if (order && inputs[0]) {
      // Legacy format
      updates.produto = {
        ...order.produto,
        precoFinal: parseFloat(inputs[0].value) || order.produto?.preco,
      };
    }

    await updateDoc(doc(db, "pedidos", currentOrderId), updates);
...
function openProductModal(p) {
  editingProduct = p;
  uploadedFiles = [];
  existingImages = p?.imagens?.length
    ? [...p.imagens]
    : p?.imagemUrl
      ? [p.imagemUrl]
      : [];
  ...
  document.getElementById("productStock").value =
    typeof p?.estoque === "number" ? p.estoque : "";
  ...
  document.querySelectorAll(".size-cb").forEach((cb) => {
    cb.checked = p?.tamanhos?.includes(cb.value) || false;
  });
...
function renderUploadGrid() {
  ...
  // Existing images (uploaded URLs)
  const existingHTML = existingImages
    .map(
      (url, i) => `
    <div class="upload-thumb" data-existing="${i}">
      <img src="${url}" alt="Foto ${i + 1}">
      ${i === 0 ? '<span class="upload-thumb-main">Principal</span>' : ""}
      <button class="upload-thumb-remove" data-existing="${i}" title="Remover">×</button>
    </div>`,
    )
    .join("");

  // New files (to be uploaded)
  const newFilesHTML = uploadedFiles
    .map((f, i) => {
      const url = URL.createObjectURL(f);
      const mainIdx = existingImages.length + i;
      return `
    <div class="upload-thumb" data-new="${i}">
      <img src="${url}" alt="Nova foto ${i + 1}">
      ${mainIdx === 0 && existingImages.length === 0 ? '<span class="upload-thumb-main">Principal</span>' : ""}
      <button class="upload-thumb-remove" data-new="${i}" title="Remover">×</button>
    </div>`;
    })
    .join("");

  grid.innerHTML = existingHTML + newFilesHTML;

  // Bind remove buttons
  grid.querySelectorAll(".upload-thumb-remove").forEach((btn) => {
    btn.addEventListener("click", (e) => {
      e.stopPropagation();
      if (btn.dataset.existing !== undefined) {
        existingImages.splice(parseInt(btn.dataset.existing), 1);
      } else if (btn.dataset.new !== undefined) {
        uploadedFiles.splice(parseInt(btn.dataset.new), 1);
      }
      renderUploadGrid();
    });
  });
  ...
  try {
    ...
    for (const file of uploadedFiles) {
      try {
        ...
        console.warn("Upload falhou:", e.message);
      }
    }
    const imagens = [...existingImages, ...uploadedUrls];
    const imagemUrl = imagens[0] || "";

    const tamanhos = [...document.querySelectorAll(".size-cb:checked")].map(
      (cb) => cb.value,
    );
